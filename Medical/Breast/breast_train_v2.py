...
def proto(datas, loader, model):
    ...
    if datas == 11:  
        ...
        #-----------------------
        embeddings_b = embeddings_b[mask_b] 
        embeddings_m = embeddings_m[mask_m]
        embeddings_n = embeddings_n[mask_n]
        #----------------------
        embeddings_b = torch.mean(embeddings_b, dim=0)
        embeddings_m = torch.mean(embeddings_m, dim=0)
        embeddings_n = torch.mean(embeddings_n, dim=0)
    
        return embeddings_b, embeddings_m, embeddings_n, embeddings_b.reshape(65,-1), embeddings_m.reshape(65,-1), embeddings_n.reshape(65,-1)
    # if datas==11:
...
class MeanDistanceLoss(nn.Module):
    ...
    def forward(self,labels,embeddings, means_b, means_m,means_n,mean_embedding_b,mean_embedding_m,mean_embedding_n):
        ...
        same_class_mask = same_class_mask.unsqueeze(2).expand(-1, -1, num_parts+1)
        #------------------
...
def train(args, tf_writer):
    ...
    for epoch in range(20):
        ...
        if accuracy_model_val > max_acc : 
            ...
            outfile = 'best_model_model_v2_val.tar'
        ...
        if accuracy_model > max_acc_mod : 
            ...
            outfile = 'best_model_model_v2_train.tar'
            torch.save({'epoch':epoch, 'state':mod.state_dict()}, outfile)

        if (epoch % 5==0) or (epoch==10-1):
            # outfile = ('{:d}.tar'.format(epoch))
            torch.save({'epoch':epoch, 'state':mod.state_dict()}, 'resnet18_model_v2.pth')
...
if __name__ == '__main__':
    ...
    tf_writer = SummaryWriter(log_dir="./runs/run1_debug_19aug_v2")
