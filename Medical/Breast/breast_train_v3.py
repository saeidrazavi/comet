...
def proto(datas, loader, model):
    ...
    if datas == 11:  
        ...
        return embeddings_b, embeddings_m, embeddings_n, embeddings_b.reshape(64,-1), embeddings_m.reshape(64,-1), embeddings_n.reshape(64,-1)
    # if datas==11:
...
def train(args, tf_writer):
    ...
    for epoch in range(40):
        ...
        if accuracy_model_val > max_acc : 
            ...
            outfile = 'best_model_model_v3_val.tar'
        ...
        if accuracy_model > max_acc_mod : 
            ...
            outfile = 'best_model_model_v3_train.tar'
            torch.save({'epoch':epoch, 'state':mod.state_dict()}, outfile)

        if (epoch % 5==0) or (epoch==10-1):
            # outfile = ('{:d}.tar'.format(epoch))
            torch.save({'epoch':epoch, 'state':mod.state_dict()}, 'resnet18_model_v3.pth')
...
if __name__ == '__main__':
    ...
    tf_writer = SummaryWriter(log_dir="./runs/run1_debug_24aug_v3")
